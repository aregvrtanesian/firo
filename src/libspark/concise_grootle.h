#ifndef FIRO_LIBSPARK_CONCISE_GROOTLE_H
#define FIRO_LIBSPARK_CONCISE_GROOTLE_H
...
public:
    ...
    bool verify(const std::vector<GroupElement>& S,
        ...
        const ConciseGrootleProof& proof); // single proof
    bool verify(const std::vector<GroupElement>& S,
        const std::vector<GroupElement>& S1,
        const std::vector<GroupElement>& V,
        const std::vector<GroupElement>& V1,
        const std::vector<std::size_t>& sizes,
        const std::vector<ConciseGrootleProof>& proofs); // batch of proofs
