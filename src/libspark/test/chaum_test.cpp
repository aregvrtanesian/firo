...
BOOST_AUTO_TEST_CASE(completeness)
{
    ...
    BOOST_CHECK(chaum.verify(S, T, proof));
}
...
BOOST_AUTO_TEST_CASE(bad_proofs)
{
    ...
    for (std::size_t i = 0; i < n; i++) {
        ...
        evil_S[i].randomize();
        BOOST_CHECK(!(chaum.verify(evil_S, T, proof)));
    }
    ...
    for (std::size_t i = 0; i < n; i++) {
        ...
        evil_T[i].randomize();
        BOOST_CHECK(!(chaum.verify(S, evil_T, proof)));
    }

    // Bad A1
    ChaumProof evil_proof = proof;
    evil_proof.A1.randomize();
    BOOST_CHECK(!(chaum.verify(S, T, evil_proof)));
