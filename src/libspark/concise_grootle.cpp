...
static bool compute_fs(
        const ConciseGrootleProof& proof,
        const Scalar& x,
...
static void compute_batch_fis(
        ...
        const Scalar& y,
        std::vector<Scalar>::iterator& ptr_S,
        std::vector<Scalar>::iterator start_ptr,
        std::vector<Scalar>::iterator end_ptr,
        std::vector<Scalar>::iterator& ptr_V,
        const Scalar& mu,
        const std::size_t n) {
    j--;
    if (j == -1)
    {
        if(ptr_S >= start_ptr && ptr_S < end_ptr){
            *ptr_S++ += f_i * y;
            *ptr_V++ += f_i * y * mu;
            f_sum += f_i;
        }
    ...
    for (std::size_t i = 0; i < n; i++)
    {
        ...
        compute_batch_fis(f_sum, t, j, f, y, ptr_S, start_ptr, end_ptr, ptr_V, mu, n);
    }
}

void ConciseGrootle::prove(
        const std::size_t l,
        const Scalar& s,
    ...
    std::size_t N = (std::size_t)std::pow(n, m); // padded input size
    ...
    Scalar mu = transcript.challenge("mu");

    proof.X.reserve(m);
    for (std::size_t j = 0; j < m; ++j)
    {
        std::vector<Scalar> P_i, mu_P_i;
        P_i.reserve(size);
        mu_P_i.reserve(size);
        for (std::size_t i = 0; i < size; ++i){
            P_i.emplace_back(P_i_j[i][j]);
            mu_P_i.emplace_back(mu * P_i_j[i][j]);
        }
        
        secp_primitives::MultiExponent mult_S(S_offset, P_i);
        secp_primitives::MultiExponent mult_V(V_offset, mu_P_i);
        proof.X.emplace_back(mult_S.get_multiple() + mult_V.get_multiple() + H*rho[j]);
    }
    ...
    // Compute zS, zV
    proof.zX = (s + mu*v) * x.exponent(uint64_t(m));
    Scalar sum;
...
bool ConciseGrootle::verify(
    ...
    std::vector<ConciseGrootleProof> proof_batch = {proof};
...
bool ConciseGrootle::verify(
        ...
        const std::vector<ConciseGrootleProof>& proofs) {
    // Sanity checks
    if (n < 2 || m < 2) {
    ...
    std::size_t N = (std::size_t)std::pow(n, m);
    ...
    for (std::size_t t = 0; t < M; t++) {
        ConciseGrootleProof proof = proofs[t];
        if (proof.X.size() != m) {
    ...
    std::vector<Scalar> S_scalars;
    std::vector<Scalar> V_scalars;
    Gi_scalars.resize(n*m);
    Hi_scalars.resize(n*m);
    S_scalars.resize(S.size());
    V_scalars.resize(V.size());

    // Set up the final batch elements
    std::vector<GroupElement> points;
    std::vector<Scalar> scalars;
    std::size_t final_size = 1 + 2*m*n + S.size() + V.size(); // F, (Gi), (Hi), (S), (V)
    for (std::size_t t = 0; t < M; t++) {
        final_size += 2 + proofs[t].X.size() + 2; // A, B, (X), S1, V1
    }
    ...
    for (std::size_t t = 0; t < M; t++) {
        ...
        // Input sets
        H_scalar += proof.zX * w2.negate();

        // Get the S scalars
        Scalar f_sum;
        Scalar f_i(uint64_t(1));
        std::vector<Scalar>::iterator ptr_S = S_scalars.begin() + S.size() - size;
        std::vector<Scalar>::iterator ptr_V = V_scalars.begin() + V.size() - size;
        compute_batch_fis(f_sum, f_i, m, f_, w2, ptr_S, ptr_S, ptr_S + size - 1, ptr_V, mu, n);
        ...
        // Finalize the scalars
        f_sum += pow;
        S_scalars[S.size() - 1] += pow * w2;
        V_scalars[V.size() - 1] += mu * pow * w2;

        // S1, V1
        points.emplace_back(S1[t]);
        scalars.emplace_back(f_sum * w2.negate());
        points.emplace_back(V1[t]);
        scalars.emplace_back(mu * f_sum * w2.negate());

        // (X)
        x_powers = Scalar(uint64_t(1));
        for (std::size_t j = 0; j < m; j++) {
            points.emplace_back(proof.X[j]);
    ...
    for (std::size_t i = 0; i < S.size(); i++) {
        ...
        scalars.emplace_back(S_scalars[i]);
    }
    for (std::size_t i = 0; i < V.size(); i++) {
        points.emplace_back(V[i]);
        scalars.emplace_back(V_scalars[i]);
    }
